defmodule Arbor.Core.DeclarativeSupervisionTest do
  ...
  @reconciler_interval 5_000  # Faster reconciliation for tests
  ...
  setup do
    ...
    cleanup_all_test_agents()
    :timer.sleep(200)  # Allow cleanup to complete
  ...
  describe "agent specifications persistence and reconciliation" do
    test "agent specifications persist across node failures" do
      ...
      end, "Agent should be restarted after reconciliation")
    ...
    test "reconciler detects and restarts missing agents" do
      ...
      end, "Missing agent should be restarted by reconciler")
    ...
    test "handles different restart strategies correctly" do
      ...
      assert {:ok, permanent_pid} = HordeSupervisor.start_agent(%{
        id: permanent_id,
      ...
      assert {:ok, temporary_pid} = HordeSupervisor.start_agent(%{
        id: temporary_id,
        module: TestRestartAgent,
      ...
      end, "Permanent agent should be restarted by reconciler")
      ...
      end, "Temporary agent spec should be removed by reconciler")
  ...
  describe "agent self-registration pattern" do
    ...
    test "agents clean up registration on normal termination" do
      ...
      case get_agent_spec_from_registry(agent_id) do
        {:error, :not_found} -> :ok  # Expected: spec was cleaned up
        {:ok, _spec} -> 
  ...
  describe "horde automatic failover validation" do
    test "agent reconciler handles process failures" do
      ...
      end, "Agent should be restarted after process failure")
      ...
      assert new_node == original_node  # In single-node test
  ...
  describe "reconciler performance and reliability" do
    test "reconciler handles large numbers of agents" do
      agent_count = 20  # Reduce count to avoid infrastructure issues
      ...
      running_count = Enum.count(agent_ids, fn agent_id ->
        case HordeSupervisor.get_agent_info(agent_id) do
      ...
      AgentReconciler.force_reconcile()
      :timer.sleep(2000)  # Allow time for all restarts
      ...
      final_running_count = Enum.count(agent_ids, fn agent_id ->
        case HordeSupervisor.get_agent_info(agent_id) do
          {:ok, info} -> Process.alive?(info.pid)
          _ -> false
        end
  ...
  defp assert_eventually_helper(fun, message, max_attempts, attempt) when attempt <= max_attempts do
    case fun.() do
