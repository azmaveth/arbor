  ...
  def determine_file_path(module_name) do
    ...
    cond do
      ...
      String.starts_with?(module_string, "Arbor.Security.") ->
        path = module_to_path(module_string, "Arbor.Security.", "apps/arbor_security/lib/arbor/security/")
        {:ok, path}
      
      String.starts_with?(module_string, "Arbor.Persistence.") ->
        path = module_to_path(module_string, "Arbor.Persistence.", "apps/arbor_persistence/lib/arbor/persistence/")
        {:ok, path}
      
      String.starts_with?(module_string, "Arbor.Contracts.") ->
        path = module_to_path(module_string, "Arbor.Contracts.", "apps/arbor_contracts/lib/arbor/contracts/")
        {:ok, path}
      ...
      true ->
        {:error, "Unable to determine file path for module #{module_string}. Module should start with 'Arbor.'"}
    end
  end
