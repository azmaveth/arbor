defmodule Arbor.Core.ClusterRegistry do
  ...
  """
  @spec list_all_agents() :: {:ok, [{Types.agent_id(), pid(), agent_metadata()}]} | {:error, term()}
  def list_all_agents() do
    ...
    case registry_impl.match({:agent, :_}, state) do
      {:ok, all_matches} ->
        agents = Enum.map(all_matches, fn {{:agent, agent_id}, pid, metadata} ->
          {agent_id, pid, metadata}
        end)
        {:ok, agents}
      error -> 
  ...
  defp get_registry_impl() do
    ...
    case Application.get_env(:arbor_core, :registry_impl, :auto) do
      ...
      :horde -> Arbor.Core.HordeRegistry
      :auto -> 
        if Mix.env() == :test do
