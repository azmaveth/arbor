defmodule Arbor.Core.ClusterEvents do
  ...
  @type event_type :: 
    ...
    :agent_restarted | :agent_migrated | :agent_reconciled |
    :cluster_node_joined | :cluster_node_left |
  ...
  def subscribe(:agent_events, opts) do
    topic = if agent_id = Keyword.get(opts, :agent_id) do
  ...
  def broadcast(event_type, event_data) 
    ...
    PubSub.broadcast(@pubsub_name, @cluster_events_topic,
      {:cluster_event, event_type, enriched_event})
  ...
  def broadcast(event_type, event_data)
      when event_type in [:reconciliation_started, :reconciliation_completed, :reconciliation_failed] do
    
    enriched_event = enrich_event(event_type, event_data)
    
    PubSub.broadcast(@pubsub_name, @reconciliation_events_topic,
      {:cluster_event, event_type, enriched_event})
  ...
  defp emit_telemetry(event_type, event_data) do
    :telemetry.execute([:arbor, :cluster_events, :broadcast], %{
      event_count: 1
    }, %{
      event_type: event_type,
      node: node(),
