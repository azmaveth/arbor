defmodule Arbor.Test.Mocks.LocalRegistry do
    ...
    Agent.get_and_update(__MODULE__, fn state ->
      case Map.get(state.groups, group) do
        ...
        members ->
          ...
          updated_groups = if updated_members == [] do
  ...
  def health_check(_state) do
    Agent.get(__MODULE__, fn state ->
      health = %{
        node_count: 1,  # Single node for mock
        registration_count: map_size(state.names),
        conflict_count: 0,  # No conflicts in single node
        nodes: [node()],
        sync_status: :healthy
      }
      {:ok, health}
    ...
    Agent.get(__MODULE__, fn state ->
      case Map.get(state.groups, group_name) do
        ...
        members when is_list(members) ->
          ...
          agent_ids = Enum.map(members, fn
            {_pid, _metadata} -> nil  # Skip tuple format
